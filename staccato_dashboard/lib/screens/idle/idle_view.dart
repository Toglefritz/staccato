...
class IdleView extends StatelessWidget {
  ...
  const IdleView(this.controller, {super.key});
  ...
  Widget build(BuildContext context) {
    ...
    final int hour12 = controller.now.hour % 12 == 0 ? 12 : controller.now.hour % 12;
    final String hour = hour12.toString().padLeft(2, '0');

    /// The minute value for the current time, with a leading zero.
    final String minute = controller.now.minute.toString().padLeft(2, '0');

    /// AM/PM indicator text.
    final String meridiem = controller.now.hour >= 12 ? 'PM' : 'AM';
              ...
              Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [controller.colorAnimation1.value!, controller.colorAnimation2.value!],
